...
if ($messages) foreach ($messages->messages as $webhook) {
  ...
  switch ($webhook_type) {
	...
	file_put_contents("php://stdout", "acknowledge!\n");
      ...
      $service_name_label = str_replace(' ', '', $service_name);
      $assignee = $webhook->data->incident->assigned_to_user->name;
      //$assigneeEmail = $webhook->data->incident->assigned_to_user->email;
      ...
      $data = array('fields'=>array('project'=>array('key'=>"$jira_project"),'summary'=>"$summary",'description'=>"*Une alerte PagerDuty a été _acknowledged_ par {$assignee}.* \n{$trigger_summary_data}. \nVous pouvez voir les détails de l'alerte à l'adresse suivante : $ticket_url", 'issuetype'=>array('name'=>"$jira_issue_type"), 'labels'=>array("pagerduty", "$service_name_label"), 'priority'=>array('name'=>"Critical"), 'assignee'=>array('name'=>"$assignee")));
	...
	file_put_contents("php://stdout", "resolve!\n");
       if(file_exists('lock.txt') && file_get_contents('lock.txt') > (time() - 5)){
        die('Should not run!');
      }
      file_put_contents('lock.txt', time());
      ...
      $service_name = $webhook->data->incident->service->name;
      $service_name_label = str_replace(' ', '', $service_name);
      $resolvedBy = $webhook->data->incident->resolved_by_user->name;
      $jiraTicket = "IN-";
      //$assigneeEmail = $webhook->data->incident->assigned_to_user->email;
      if ($webhook->data->incident->trigger_summary_data->subject) {
        $trigger_summary_data = $webhook->data->incident->trigger_summary_data->subject;
      ...
      $verb = "resolved";
		file_put_contents("php://stdout", $resolvedBy . "\n");
      //Il faut trouver le numéro de billet JIRA. 
      file_put_contents("php://stdout", "cherche billet jira!\n");
      $url = "https://$pd_subdomain.pagerduty.com/api/v1/incidents/$incident_id/notes";
      $return = http_request($url, "", "GET", "token", "", $pd_api_token);
      if ($return['status_code'] == '200') {
        $response = json_decode($return['response'], true);
        if (array_key_exists("notes", $response)) {
          foreach ($response['notes'] as $value) {
            $startsWith = "JIRA ticket";
            $theNote = $value['content'];
            
            if (substr($theNote, 0, strlen($startsWith)) === $startsWith) {
         		//$jiraTicket = find_between($theNote, $startsWith, 'has');
         		preg_match("/(?<=ticket).*?(?=has)/s", $theNote, $result);
         		$jiraTicket = $result[0];
         		$jiraTicket = str_replace(' ', '', $jiraTicket);
            }
            file_put_contents("php://stdout", $jiraTicket);
          }
        }
      }
      //Ajout d'un commentaire au billet JIRA trouvé précédemment. 
      $url = "$jira_url/rest/api/2/issue/$jiraTicket";
	
	if($resolvedBy == '') {
      $data = array('update'=>array('comment'=>array(array('add'=>array('body'=>"L'alerte PagerDuty s'est résolue par elle-même.")))));
    }
    else {
    $data = array('update'=>array('comment'=>array(array('add'=>array('body'=>"L'alerte PagerDuty s'est résolue par {$resolvedBy}.")))));
	}
	$data_json = json_encode($data);
      $return = http_request($url, $data_json, "PUT", "basic", $jira_username, $jira_password);
      $status_code = $return['status_code'];
      $response = $return['response'];
      $response_obj = json_decode($response);
      $response_key = $response_obj->key;
      ...
      if ($status_code == "201") {
        //Update the PagerDuty ticket with the JIRA ticket information.
        $url = "https://$pd_subdomain.pagerduty.com/api/v1/incidents/$incident_id/notes";
        $data = array('note'=>array('content'=>"JIRA ticket $response_key has been updated.  You can view it at $jira_url/browse/$response_key."),'requester_id'=>"$pd_requester_id");
        $data_json = json_encode($data);
        http_request($url, $data_json, "POST", "token", "", $pd_api_token);
      }
      else {
    ...
    default:
    file_put_contents("php://stdout", "default!\n");
      continue;
  }
}
